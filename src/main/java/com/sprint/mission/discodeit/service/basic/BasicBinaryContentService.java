...
public class BasicBinaryContentService implements BinaryContentService {
  ...
  public BinaryContentDto updateStatus(UUID binaryContentId, BinaryContentStatus status) {
    ...
    BinaryContent binaryContent = binaryContentRepository.findById(binaryContentId)
        .orElseThrow(() -> BinaryContentNotFoundException.withId(binaryContentId));
    BinaryContentDto from = binaryContentMapper.toDto(binaryContent); //상태 변경 전

    binaryContent.updateStatus(status);
    binaryContentRepository.save(binaryContent);

    BinaryContentDto to = binaryContentMapper.toDto(binaryContent); //상태 변경 후

    eventPublisher.publishEvent(BinaryContentUpdatedEvent.now(from, to));
    return to;
  }
}
