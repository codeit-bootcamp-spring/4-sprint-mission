...
public class S3BinaryContentStorage implements BinaryContentStorage {
  ...
  public UUID put(UUID binaryContentId, byte[] bytes) {
    ...
    try {
      ...
      log.warn("S3에 파일 업로드 실패. 재시도 예정: key = {}, message = {} ", key, e.getMessage());
      throw e;
    }
  }
  ...
  public UUID recover(S3Exception e, UUID binaryContentId, byte[] bytes) {
    log.error("S3 업로드 완전 실패 key = {} 모든 재시도 실패", binaryContentId, e);

    logToSystem(binaryContentId, e);

    basicNotificationService.sendAdminAlert(
        "S3 업로드",
  ...
  private void logToSystem(UUID binaryContentId, Exception e) {
    // 추가적인 에러 로깅 시스템 연동 가능 (ex: DB, Sentry 등)
    log.error("[S3_UPLOAD_FAILURE] id={} reason={}", binaryContentId, e.getMessage());
  }
