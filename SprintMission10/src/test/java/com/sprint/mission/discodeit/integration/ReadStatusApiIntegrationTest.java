...
class ReadStatusApiIntegrationTest {
  ...
  void createReadStatus_Success() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/readStatuses").contentType(MediaType.APPLICATION_JSON).content(requestBody))
  ...
  void createReadStatus_Failure_Duplicate() throws Exception {
    ...
    // 테스트 사용자 생성
    UserCreateRequest userRequest =
        new UserCreateRequest("duplicateuser", "duplicate@example.com", "Password1!");
    UserDto user = userService.create(userRequest, Optional.empty());

    // 공개 채널 생성
    PublicChannelCreateRequest channelRequest =
        new PublicChannelCreateRequest("중복 테스트 채널", "중복 테스트 채널 설명입니다.");
    ChannelDto channel = channelService.create(channelRequest);

    // 첫 번째 읽음 상태 생성 요청 (성공)
    Instant lastReadAt = Instant.now();
    ReadStatusCreateRequest firstCreateRequest =
        new ReadStatusCreateRequest(user.id(), channel.id(), lastReadAt);
    ...
    ReadStatusCreateRequest duplicateCreateRequest =
        new ReadStatusCreateRequest(user.id(), channel.id(), Instant.now());
  ...
  void updateReadStatus_Success() throws Exception {
    ...
    // 테스트 사용자 생성
    UserCreateRequest userRequest =
        new UserCreateRequest("updateuser", "update@example.com", "Password1!");
    UserDto user = userService.create(userRequest, Optional.empty());

    // 공개 채널 생성
    PublicChannelCreateRequest channelRequest =
        new PublicChannelCreateRequest("업데이트 테스트 채널", "업데이트 테스트 채널 설명입니다.");
    ChannelDto channel = channelService.create(channelRequest);

    // 읽음 상태 생성
    Instant initialLastReadAt = Instant.now().minusSeconds(3600); // 1시간 전
    ReadStatusCreateRequest createRequest =
        new ReadStatusCreateRequest(user.id(), channel.id(), initialLastReadAt);
  ...
  void updateReadStatus_Failure_NotFound() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/readStatuses/{readStatusId}", nonExistentReadStatusId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(requestBody))
  ...
  void findAllReadStatusesByUserId_Success() throws Exception {
    ...
    // 테스트 사용자 생성
    UserCreateRequest userRequest =
        new UserCreateRequest("listuser", "list@example.com", "Password1!");
    UserDto user = userService.create(userRequest, Optional.empty());

    // 여러 채널 생성
    PublicChannelCreateRequest channelRequest1 =
        new PublicChannelCreateRequest("목록 테스트 채널 1", "목록 테스트 채널 설명입니다.");

    PublicChannelCreateRequest channelRequest2 =
        new PublicChannelCreateRequest("목록 테스트 채널 2", "목록 테스트 채널 설명입니다.");

    ChannelDto channel1 = channelService.create(channelRequest1);
    ChannelDto channel2 = channelService.create(channelRequest2);

    // 각 채널에 대한 읽음 상태 생성
    ReadStatusCreateRequest createRequest1 =
        new ReadStatusCreateRequest(user.id(), channel1.id(), Instant.now().minusSeconds(3600));

    ReadStatusCreateRequest createRequest2 =
        new ReadStatusCreateRequest(user.id(), channel2.id(), Instant.now());
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath(
                "$[*].channelId", hasItems(channel1.id().toString(), channel2.id().toString())));
  }
}
