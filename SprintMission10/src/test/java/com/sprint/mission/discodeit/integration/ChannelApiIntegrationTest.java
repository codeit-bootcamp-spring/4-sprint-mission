...
class ChannelApiIntegrationTest {
  ...
  void createPublicChannel_Failure_InvalidRequest() throws Exception {
    // Given
    PublicChannelCreateRequest invalidRequest =
  ...
  void findAllChannelsByUserId_Success() throws Exception {
    ...
    UserCreateRequest userRequest =
        new UserCreateRequest("channeluser", "channeluser@example.com", "Password1!");

    UserDto user = userService.create(userRequest, Optional.empty());
    UUID userId = user.id();

    // 공개 채널 생성
    PublicChannelCreateRequest publicChannelRequest =
        new PublicChannelCreateRequest("공개 채널 1", "공개 채널 설명입니다.");

    channelService.create(publicChannelRequest);

    // 비공개 채널 생성
    UserCreateRequest otherUserRequest =
        new UserCreateRequest("otheruser", "otheruser@example.com", "Password1!");

    UserDto otherUser = userService.create(otherUserRequest, Optional.empty());

    PrivateChannelCreateRequest privateChannelRequest =
        new PrivateChannelCreateRequest(List.of(userId, otherUser.id()));
  ...
  void updateChannel_Success() throws Exception {
    ...
    // 공개 채널 생성
    PublicChannelCreateRequest createRequest =
        new PublicChannelCreateRequest("원본 채널", "원본 채널 설명입니다.");
    ...
    PublicChannelUpdateRequest updateRequest =
        new PublicChannelUpdateRequest("수정된 채널", "수정된 채널 설명입니다.");
  ...
  void updateChannel_Failure_ChannelNotFound() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/channels/{channelId}", nonExistentChannelId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(requestBody))
  ...
  void deleteChannel_Success() throws Exception {
    ...
    // 공개 채널 생성
    PublicChannelCreateRequest createRequest =
        new PublicChannelCreateRequest("삭제할 채널", "삭제할 채널 설명입니다.");
    ...
    UserCreateRequest userRequest =
        new UserCreateRequest("testuser", "testuser@example.com", "Password1!");
  ...
  void deleteChannel_Failure_ChannelNotFound() throws Exception {
    ...
    // When & Then
    mockMvc
        .perform(delete("/api/channels/{channelId}", nonExistentChannelId))
        .andExpect(status().isNotFound());
  }
}
