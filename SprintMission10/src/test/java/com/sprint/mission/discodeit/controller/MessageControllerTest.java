...
class MessageControllerTest {
  ...
  void createMessage_Success() throws Exception {
    ...
    MessageCreateRequest createRequest =
        new MessageCreateRequest("안녕하세요, 테스트 메시지입니다.", channelId, authorId);
    ...
    MockMultipartFile attachment =
        new MockMultipartFile(
            "attachments", "test.jpg", MediaType.IMAGE_JPEG_VALUE, "test-image".getBytes());

    UUID messageId = UUID.randomUUID();
    Instant now = Instant.now();

    UserDto author = new UserDto(authorId, "testuser", "test@example.com", null, true);

    BinaryContentDto attachmentDto =
        new BinaryContentDto(UUID.randomUUID(), "test.jpg", 10L, MediaType.IMAGE_JPEG_VALUE);

    MessageDto createdMessage =
        new MessageDto(
            messageId, now, now, "안녕하세요, 테스트 메시지입니다.", channelId, author, List.of(attachmentDto));
  ...
  void deleteMessage_Success() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/api/messages/{messageId}", messageId).contentType(MediaType.APPLICATION_JSON))
  ...
  void deleteMessage_Failure_MessageNotFound() throws Exception {
    ...
    willThrow(MessageNotFoundException.withId(nonExistentMessageId))
        ...
        .delete(nonExistentMessageId);

    // When & Then
    mockMvc
        .perform(
            delete("/api/messages/{messageId}", nonExistentMessageId)
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isNotFound());
  }
